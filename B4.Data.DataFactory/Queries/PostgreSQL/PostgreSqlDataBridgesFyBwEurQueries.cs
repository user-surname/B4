...
{
    public sealed class PostgreSqlDataBridgesFyBwEurQueries : IDataBridgesFyBwEurQueries
    {
        public string AddQuery() => @"
            ...
            (idapicarga, guidcarga, fechaultmodif, idcompany, ejercicio, idciclo, idfase, idcurrency, idepigrafe,
             ...
             prototooling, other, ""check"", comments, idcarga, idcargastgbw, idhoja)
            VALUES
            (@IdAPICarga, @GuidCarga, @FechaUltModif, @IdCompany, @Ejercicio, @IdCiclo, @IdFase, @IdCurrency, @IdEpigrafe,
             @Actuals, @PctActuals, @Budget, @PctBudget, @Variance,
             @Volume, @InventoryChange, @Mix, @New, @Economics, @QuickSavings,
        ...
        public string GetByIdQuery() => "SELECT * FROM b4.data_bridges_fy_bw_eur WHERE id = @Id";

        public string GetAllQuery() => "SELECT * FROM b4.data_bridges_fy_bw_eur LIMIT 2000";
        ...
        public string DeleteQuery() => "DELETE FROM b4.data_bridges_fy_bw_eur WHERE id = @Id";

        public string GetByPlantaEjercicioQuery() => @"
            SELECT * FROM b4.data_bridges_fy_bw_eur
