...
{
    public sealed class PostgreSqlStgDataComentariosQueries : IStgDataComentariosQueries
    {
        public string AddQuery() => @"
            INSERT INTO b4.stg_data_comentarios
            (idapicarga, guidcarga, fechaultmodif, idcompany, ejercicio, idciclo, idfase, idepigrafe, etiqueta, comentario)
            VALUES
            (@IdAPICarga, @GuidCarga, @FechaUltModif, @IdCompany, @Ejercicio, @IdCiclo, @IdFase, @IdEpigrafe, @Etiqueta, @Comentario)
            RETURNING id";

        public string GetByIdQuery() => "SELECT * FROM b4.stg_data_comentarios WHERE id = @Id";

        public string GetAllQuery() => "SELECT * FROM b4.stg_data_comentarios LIMIT 2000";

        public string UpdateQuery() => @"
            UPDATE b4.stg_data_comentarios SET
        ...
        public string DeleteQuery() => "DELETE FROM b4.stg_data_comentarios WHERE id = @Id";
    }
}
