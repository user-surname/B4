using B4.Models.Entities.LkEntities;
using B4.Models.RepositoryInterfaces.LkInterfaces;
using B4.Models.ServiceInterfaces;

namespace B4.Domain.Services
{
    public class PlantTreeService : IPlantTreeService
    {
        private readonly IPlantTreeRepository _repository;

        public PlantTreeService(IPlantTreeRepository repository)
        {
            _repository = repository;
        }

        public async Task<LkPlantTree> GetByIdAsync(int id)
        {
            return await _repository.GetByIdAsync(id);
        }

        public async Task<IEnumerable<LkPlantTree>> GetAllAsync()
        {
            return await _repository.GetAllAsync();
        }

        public async Task AddAsync(LkPlantTree entity)
        {
            await _repository.AddAsync(entity);
        }

        public async Task DeleteAsync(int id)
        {
            var existing = await _repository.GetByIdAsync(id);
            if (existing == null)
                throw new Exception($"No existe árbol con id={id}");

            await _repository.DeleteAsync(id);
        }
    }
}
