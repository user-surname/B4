...
public class DataBridgesFyRepositoryTest : IDisposable
{
    ...
    public DataBridgesFyRepositoryTest()
    {
        ...
        _repository = new DataBridgesFyRepository(_context, _queryProvider);
    }
    ...
    public async Task AddAsync_ShouldInsertRecord()
    {
        ...
        var result = conn2.QuerySingleOrDefault<DataBridgesFy>("SELECT * FROM DATA_BridgesFY WHERE Id = @id", new { id = newId });
    ...
    public async Task GetAllAsync_ShouldReturnAllRecords()
    {
        ...
        using (var conn = _context.CreateConnection())
        {
            id1 = conn.ExecuteScalar<int>(@"
                INSERT INTO DATA_BridgesFY (IdAPICarga, guidCarga, FechaUltModif, IdCompany, Ejercicio, IdCiclo, IdFase, IdCurrency, IdEpigrafe, FiscalYear, Percentage, Zero, ZeroPercentage, Absolute, AbsolutePercentage, VMixNew, RawMaterial, Scrap, Economics, CurrencyMix, Performance, ProtoTool, Others, Comments)
    ...
    public async Task UpdateAsync_ShouldModifyRecord()
    {
        ...
        using var conn2 = _context.CreateConnection();
        var result = conn2.QuerySingle<DataBridgesFy>("SELECT * FROM DATA_BridgesFY WHERE Id = @id", new { id = newId });
