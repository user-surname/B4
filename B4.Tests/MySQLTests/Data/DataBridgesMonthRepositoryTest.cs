...
namespace B4.Tests.MySQLTests.Data
{
    ...
    public class DataBridgesMonthRepositoryTest : IDisposable
    {
        ...
        public DataBridgesMonthRepositoryTest()
        {
            ...
            _repository = new DataBridgesMonthRepository(_context, _queryProvider);
        }
        ...
        public void Dispose()
        {
            ...
            conn.Execute("DELETE FROM DATA_BridgesMonth WHERE Id IN @ids", new { ids = _insertedIds });
            _insertedIds.Clear();
        }
        ...
        {
            var record = new DataBridgesMonth
            {
                IdAPICarga = 1,
            ...
            using (var conn = _context.CreateConnection())
            {
                newId = conn.ExecuteScalar<int>("SELECT Id FROM DATA_BridgesMonth ORDER BY Id DESC LIMIT 1;");
            }

            _insertedIds.Add(newId);

            using var conn2 = _context.CreateConnection();
            var result = conn2.QuerySingleOrDefault<DataBridgesMonth>("SELECT * FROM DATA_BridgesMonth WHERE Id = @id", new { id = newId });
        ...
        public async Task UpdateAsync_ShouldModifyRecord()
        {
            ...
            var updated = new DataBridgesMonth
            {
                Id = newId,
                Volume = 999,
                Comments = "Updated"
            ...
            using var conn2 = _context.CreateConnection();
            var result = conn2.QuerySingle<DataBridgesMonth>("SELECT * FROM DATA_BridgesMonth WHERE Id = @id", new { id = newId });
