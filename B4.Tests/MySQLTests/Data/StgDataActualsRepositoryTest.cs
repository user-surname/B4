...
{
    public class StgDataActualsRepositoryTest : IDisposable
    {
        private readonly IDbConnectionFactory _context;
        private readonly IDataQueryProvider _queryProvider;
        private readonly StgDataActualsRepository _repository;
        private readonly List<int> _insertedIds = new();

        public StgDataActualsRepositoryTest()
        {
            ...
            _repository = new StgDataActualsRepository(_context, _queryProvider);
        }
        ...
        public void Dispose()
        {
            ...
            conn.Execute("DELETE FROM STG_DATA_Actuals WHERE id IN @ids", new { ids = _insertedIds });
            _insertedIds.Clear();
        }
        ...
        public async Task AddAsync_ShouldInsertRecord()
        {
            ...
            using (var conn = _context.CreateConnection())
            {
                newId = conn.ExecuteScalar<int>("SELECT id FROM STG_DATA_Actuals ORDER BY id DESC LIMIT 1;");
            }

            _insertedIds.Add(newId);

            using var conn2 = _context.CreateConnection();
            var result = conn2.QuerySingleOrDefault<StgDataActuals>("SELECT * FROM STG_DATA_Actuals WHERE id = @id", new { id = newId });
        ...
        public async Task UpdateAsync_ShouldModifyRecord()
        {
            ...
            var updated = new StgDataActuals
            {
                Id = newId,
                Mes00 = 999,
                Mes13 = 888
            ...
            using var conn2 = _context.CreateConnection();
            var result = conn2.QuerySingle<StgDataActuals>("SELECT * FROM STG_DATA_Actuals WHERE id = @id", new { id = newId });
