...
public class StgDataBudgetRepositoryTest : IDisposable
{
    ...
    public StgDataBudgetRepositoryTest()
    {
        ...
        _repository = new StgDataBudgetRepository(_context, _queryProvider);
    }
    ...
    public void Dispose()
    {
        ...
        conn.Execute("DELETE FROM STG_DATA_Budget WHERE id IN @ids", new { ids = _insertedIds });
        _insertedIds.Clear();
    }
    ...
    public async Task AddAsync_ShouldInsertRecord()
    {
        ...
        using (var conn = _context.CreateConnection())
        {
            newId = conn.ExecuteScalar<int>("SELECT id FROM STG_DATA_Budget ORDER BY id DESC LIMIT 1;");
        }

        _insertedIds.Add(newId);

        using var conn2 = _context.CreateConnection();
        var result = conn2.QuerySingleOrDefault<StgDataBudget>("SELECT * FROM STG_DATA_Budget WHERE id = @id", new { id = newId });
    ...
    public async Task UpdateAsync_ShouldModifyRecord()
    {
        ...
        using var conn2 = _context.CreateConnection();
        var result = conn2.QuerySingle<StgDataBudget>("SELECT * FROM STG_DATA_Budget WHERE id = @id", new { id = newId });
