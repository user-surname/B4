...
public class StgDataComentariosRepositoryTest : IDisposable
{
    ...
    public StgDataComentariosRepositoryTest()
    {
        ...
        _repository = new StgDataComentariosRepository(_context, _queryProvider);
    }
    ...
    public void Dispose()
    {
        ...
        conn.Execute("DELETE FROM STG_DATA_Comentarios WHERE Id IN @ids", new { ids = _insertedIds });
        _insertedIds.Clear();
    }
    ...
    {
        var comentario = new StgDataComentarios
        {
            IdAPICarga = 1,
        ...
        using (var conn = _context.CreateConnection())
        {
            newId = conn.ExecuteScalar<int>("SELECT Id FROM STG_DATA_Comentarios ORDER BY Id DESC LIMIT 1;");
        }

        _insertedIds.Add(newId);

        using var conn2 = _context.CreateConnection();
        var result = conn2.QuerySingleOrDefault<StgDataComentarios>("SELECT * FROM STG_DATA_Comentarios WHERE Id = @id", new { id = newId });
    ...
    public async Task UpdateAsync_ShouldModifyRecord()
    {
        ...
        var updated = new StgDataComentarios
        {
            Id = newId,
            IdAPICarga = 1,
            GuidCarga = Guid.NewGuid(),
        ...
        using var conn2 = _context.CreateConnection();
        var result = conn2.QuerySingle<StgDataComentarios>("SELECT * FROM STG_DATA_Comentarios WHERE Id = @id", new { id = newId });
